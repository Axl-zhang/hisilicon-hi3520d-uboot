...
#define NAND_CMD_READ2		2
#define NAND_CMD_ASYN_READ	0x32
#endif
...
struct nand_chip {
	...
	unsigned int async_work_flag;
	unsigned int current_id;
	unsigned int total_status;
	struct list_head list;
	unsigned int work_no;
	unsigned int total_work_no;
	void (*enable_int)(struct nand_chip *chip, unsigned int op);
};
...
struct async_work_info {
	...
	unsigned char *addr;
	void *private;
	int (*complete)(int read_result, void *private);
};

extern int nand_async_read(struct mtd_info *mtd, loff_t from, size_t len,
		uint8_t *buf);
