...
#ifdef HDMI_SCDC_SUPPORT
        SiiDrvTxScdcStatusGet(pObj->scdcInst, &(((SiiHdmiStatus_t*)outData)->scdcStatus));
#endif
        break;
#endif
...
static bool_t sClearI2cDeadlock(HdmiTxObj_t* pObj)
{
    ...
    if (timeout > 0 && (BIT_MSK__DDC_MANUAL__IO_DSDA & SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__DDC_MANUAL)))
    {
        ...
        SII_LIB_LOG_WARN(pObj, "Clear I2C deadlock success\n");
        return true;
    }
    else
    {
        SII_LIB_LOG_ERR(pObj, "Clear I2C deadlock fail\n");
        return false;
    }
}
//-------------------------------------------------------------------------------------------------
...
static ddcComErr_t sUpdateEdid(HdmiTxObj_t* pObj, uint8_t segmentIndex, uint8_t regAddr, uint8_t *pBuf, uint16_t length)
{
	...
	do
	{
	 ...
	 read_again:
        ...
        SII_LIB_LOG_DEBUG1(pObj, "TPI_HW_DBG5:0x%x\n", SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | 0x367D));
        SII_LIB_LOG_DEBUG1(pObj, "TPI_HW_DBG6:0x%x\n", SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | 0x367E));
        SII_LIB_LOG_DEBUG1(pObj, "TPI_HW_DBG7:0x%x\n", SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | 0x367F));
        SII_LIB_LOG_DEBUG1(pObj, "DDC_STATUS:0x%x\n", SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | 0x30F2));
        SII_LIB_LOG_DEBUG1(pObj, "DDC_FIFO_CNT:0x%x\n", SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__DDC_DOUT_CNT));
	...
	if( dsDdcError )
	{
	    SII_LIB_LOG_ERR(pObj, "sUpdateEdid fail!!,dsDdcError=0x%x\n", dsDdcError);
		SiiDrvCraWrReg8(craInst, baseAddr | REG_ADDR__DDC_CMD, BIT_ENUM__DDC_CMD__ABORT_TRANSACTION);
	}
...
static ddcComErr_t sEdidRead(HdmiTxObj_t* pObj)
{
    ...
    {
        SII_LIB_LOG_ERR(pObj, "TPI_HW_DBG5:0x%x\n", SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | 0x367D));
        SII_LIB_LOG_ERR(pObj, "TPI_HW_DBG6:0x%x\n", SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | 0x367E));
        SII_LIB_LOG_ERR(pObj, "TPI_HW_DBG7:0x%x\n", SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | 0x367F));
        SII_LIB_LOG_ERR(pObj, "DDC_STATUS:0x%x\n", SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | 0x30F2));
        SII_LIB_LOG_ERR(pObj, "DDC_FIFO_CNT:0x%x\n", SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__DDC_DOUT_CNT));
        SII_LIB_LOG_ERR(pObj, "EDID read fail!!\n");
    }
	...
	if(SI_TX_DDC_ERROR_CODE_NO_ERROR != sUpdateEdid(pObj, 0xFF, 0x50, &pObj->dsHdcp_2_2_Supported, 1))
	{
		...
		{
			//SiiDrvTxDdcReset();
			SII_LIB_LOG_ERR(pObj, "Error happened during HDCP2.2 Capability read!!\n");
		}
		else
		{
			SII_LIB_LOG_DEBUG1(pObj, "HDCP2.2 capability has been read successfully!!\n");
		}
	}
	else
	{
...
static void sAudioEnableSet(HdmiTxObj_t* pObj, uint8_t enable)
{
    if (enable)
    {
        ...
        SiiDrvCraSetBit8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__AUD_EN, BIT_MSK__AUD_EN__REG_AUD_SEL_OWRT); // HBRA authentication pass
    }
    else
    {
