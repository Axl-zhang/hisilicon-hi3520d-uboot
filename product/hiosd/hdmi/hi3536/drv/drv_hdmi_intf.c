...
{
    //const HDMI_SINK_CAPABILITY_S   *pstSinkCap;
    ...
    if ((stHwStatus.stScdcStatus.bSourceScrambleOn &&
        stHwStatus.stScdcStatus.bSinkScrambleOn && 
        stHwStatus.stScdcStatus.u8TmdsBitClkRatio == 40 && bEnable) ||
        (!stHwStatus.stScdcStatus.bSourceScrambleOn &&
        !stHwStatus.stScdcStatus.bSinkScrambleOn && 
        stHwStatus.stScdcStatus.u8TmdsBitClkRatio == 10 && !bEnable))
    {
        HDMI_INFO("Scramble status is not change, so no need to config scramble\n");
        return HI_SUCCESS;
    }  

    HDMI_INFO("SourceScrambleOn:%d, SinkScrambleOn:%d, TmdsBitClkRatio:%d\n",stHwStatus.stScdcStatus.bSourceScrambleOn,stHwStatus.stScdcStatus.bSinkScrambleOn,stHwStatus.stScdcStatus.u8TmdsBitClkRatio);
...
{   
    ...
    if (bHotPlug)
    {
        ...
        { 
            DRV_HDMI_EdidCapabilityGet(&pstHdmiDev->stEdidInfo, &pstSinkCap);  
        ...
        case HDMI_EVENT_HOTPLUG:
            {
                ...
                DRV_HDMI_EdidUpdate(&pstHdmiDev->stEdidInfo, HDMI_EDID_UPDATE_SINK);
...
static HI_S32 HdmiFileOpen(struct inode *inode, struct file *filp)
{
    ...
    if (1 == atomic_inc_return(&s_HdmiCount))
    {
...
HI_S32 DRV_HDMI_CmdProcess(unsigned int cmd, HI_VOID *arg, HI_BOOL bUser)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_HDMI_GET_SINK_CAPABILITY:
        {
            ...
            {
                HDMI_ERR("Get sink capabity fail\n");
                s32Ret = HI_FAILURE;
            }
            break;
        }
...
HI_S32 HDMI_DRV_ModInit(HI_VOID)
{
    ...
    if (HI_SUCCESS != HI_HDMI_DEV_Register(s_stHdmiDev))
    {
        printk("HI_HDMI_DEV_Register hdmi fail \n");
        return HI_FAILURE;
    }
