			    ...
			    int column, int page_addr)
{
	...
	case NAND_CMD_READ2:
		chip->cmd_ctrl(mtd, NAND_CMD_ASYN_READ,
...
#ifdef CONFIG_SUPPORT_DMA_INT
int nand_async_read(struct mtd_info *mtd, loff_t from, size_t len, uint8_t *buf)
{
	unsigned chip_cs, real_page;
	struct nand_chip *chip = mtd->priv;

	if ((from + len) > mtd->size) {
		printf("%s error: Area is out of range\n", __func__);
		return -EINVAL;
	}

	if (!len) {
		printf("%s error: Invalid argument, length is zero\n",
				__func__);
		return -EINVAL;
	}
	...
	chip_cs = (int)(from >> chip->chip_shift);
	chip->select_chip(mtd, chip_cs);

	real_page = (int)(from >> chip->page_shift);

	chip->cmdfunc(mtd, NAND_CMD_READ2, 0x00, (real_page & chip->pagemask));
...
#elif defined(CONFIG_HIFMC_SPI_NAND_SUPPORT)
			nand_info_ex.id_length = MAX_SPI_NAND_ID_LEN;
#else
