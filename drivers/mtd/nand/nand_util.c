...
void work_info_show(struct nand_chip *chip)
{
	struct list_head *entry;
	struct async_work_info *tmp;

	printf("{[ head ]} work number:%d current id:%d\n", chip->work_no,
			chip->current_id);
	printf("|\n");
	list_for_each(entry, &chip->list) {
		tmp = list_entry(entry, struct async_work_info, link);

		printf("{[id:%d off(%#x) len(%#x) addr(%#x) complete(%#x)]}\n",
				tmp->id, (unsigned)tmp->off,
				(unsigned)tmp->len, (unsigned)tmp->addr,
				(unsigned)tmp->complete);
		printf("|\n");
	}
	printf("{[ end ]}\n");
}

struct async_work_info *work_info_find_by_id(struct nand_chip *chip,
		unsigned int id)
{
	struct list_head *entry;
	struct async_work_info *work_tmp;

	list_for_each(entry, &chip->list) {
		work_tmp = list_entry(entry, struct async_work_info, link);
...
int nand_async_read_skip_bad(nand_info_t *nand, loff_t off, size_t *len,
		unsigned char *buf)
{
	int rval;
	size_t len_incl_bad;
	...
	u_char flag_first = 1;
	struct nand_chip *chip = nand->priv;
	struct async_work_info *new;
	struct async_work_info *old = NULL;
	struct async_work_info temp;

	len_incl_bad = get_len_incl_bad(nand, off, *len);

	if ((off + len_incl_bad) > nand->size) {
		printf("Attempt to read outside the flash area\n");
		return -EINVAL;
	}
	...
	if ((len_incl_bad == *len)
	    || ((!(off & (nand->erasesize - 1)))
		    && (*len < nand->erasesize))) {
		rval = nand_async_read(nand, off, (size_t)*len, (u_char *)buf);
		if (!rval || rval == -EUCLEAN)
			return 0;
		printf("NAND read from off %llx failed %d\n", off, rval);
		return rval;
	}

	while (left_to_read > 0) {
		size_t block_off = off & (nand->erasesize - 1);
		size_t read_len;

		WATCHDOG_RESET();

		if (nand_block_isbad(nand,
				off & ~((loff_t)nand->erasesize - 1))) {
			printf("Skipping bad block 0x%08llx\n",
				off & ~((loff_t)nand->erasesize - 1));
		...
		if (left_to_read < (nand->erasesize - block_off))
			read_len = left_to_read;
		else
			read_len = nand->erasesize - block_off;

		if (flag_first) {
			old = work_info_find_by_id(chip, chip->current_id);
			if (old == NULL) {
			...
			raw_off = old->off;
			raw_len = old->len;
			memcpy((void *)&temp, (void *)old,
					sizeof(struct async_work_info));

			/* disable DMA interrupt */
			chip->enable_int(chip, 0);
			...
			chip->enable_int(chip, 1);
		}

		/* allocate memory async work info structure */
		new = (struct async_work_info *)
			malloc(sizeof(struct async_work_info));
		if (new == NULL) {
			printf("can't allocate memory\n");
			return -ENOMEM;
		}

		new->id = ++chip->total_work_no;
		new->off = off;
		new->len = read_len;
		new->addr = p_buf;
		new->private = temp.private;

		if (off + read_len == raw_off + raw_len)
			new->complete = temp.complete;
		else
			new->complete = NULL;

		if (flag_first) {
			chip->current_id = new->id;
			temp.off = off;
			temp.len = read_len;
			temp.addr = p_buf;
			flag_first = 0;
		...
		list_add_tail(&new->link, &chip->list);
		chip->work_no++;
	...
	rval = nand_async_read(nand, temp.off, temp.len, temp.addr);
	if (rval && rval != -EUCLEAN) {
		printf("NAND read from off %llx failed %d\n", off, rval);
		*len -= left_to_read;
		return rval;
...
int nand_async_read_list(nand_info_t *nand, loff_t offset, size_t *length,
		u_char *buffer, int (*complete)(int, void *), void *private)
{
	struct nand_chip *chip = nand->priv;
	struct async_work_info *new = NULL;
	struct async_work_info *work;

	if (!chip->complete_flag && !chip->work_no) {
		printf("%s error: complete flag %d but work number is %d.\n",
			__func__, chip->complete_flag, chip->work_no);
		return -EINVAL;
	}

	/* first time and add new work when work running */
	if ((chip->complete_flag && !chip->work_no)
	    || (chip->work_no && !chip->complete_flag)) {

		if ((offset > nand->size) || !length || !buffer || !complete) {
	...
	/* only add work when work is running */
	chip->enable_int(chip, 0);
	...
	if (!chip->work_no) {
		printf("%s error: start work but not work in list\n",
				__func__);
		return -EINVAL;
	}

	/* start before add work */
	if (new != NULL) {
		if (chip->work_no == 1) {
			chip->current_id = new->id;
			work = new;
		...
		if (!chip->current_id) {
			printf("%s error: current id is %d!\n", __func__,
				chip->current_id);
			return -EINVAL;
		}

		/* start next work */
		work = work_info_find_by_id(chip, chip->current_id);
		if (work == NULL) {
			printf("%s error: Can't find current work %d!\n",
					__func__,
					chip->current_id);
			work_info_show(chip);
			return -EINVAL;
	...
	chip->complete_flag = 0;
	chip->async_work_flag = 1;
	chip->total_status = 0;

	nand_async_read_skip_bad(nand, work->off, &work->len, work->addr);
