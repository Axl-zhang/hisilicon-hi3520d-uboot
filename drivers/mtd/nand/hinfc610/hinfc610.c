...
void hinfc610_enable_int(struct nand_chip *chip, unsigned int op);
void hinfc_irq_handle(void *arg);
extern void work_info_show(struct nand_chip *chip);
extern int irq_connect(unsigned irqnr, void (*handle)(void *), void *arg);
#ifdef CONFIG_SUPPORT_DMA_INT
extern struct async_work_info *work_info_find_by_id(struct nand_chip *chip,
		unsigned int id);
extern int nand_async_read_list(nand_info_t *nand, loff_t offset,
		size_t *length, u_char *buffer, int (*complete)(int, void *),
		void *private);
#endif
...
static void hinfc610_cmd_ctrl(struct mtd_info *mtd, int dat, unsigned int ctrl)
{
	...
	if ((ctrl & NAND_CLE) && (ctrl & NAND_CTRL_CHANGE)) {
		...
		case NAND_CMD_RESET:
			...
			break;
#ifdef CONFIG_SUPPORT_DMA_INT
		case NAND_CMD_ASYN_READ:
			host->send_cmd_async_read(host);
			return;
#endif
...
{
	unsigned temp_async_work_flag = 0;
	struct nand_chip *chip = host->chip;
	...
	*host->epm = 0x0000;
	...
	if (chip->async_work_flag) {
		temp_async_work_flag = chip->async_work_flag;
		chip->async_work_flag = 0;
	}

	hinfc610_dma_transfer(host, 1);

	chip->async_work_flag = temp_async_work_flag;
...
#ifdef CONFIG_SUPPORT_DMA_INT
static int hinfc610_send_cmd_async_read(struct hinfc_host *host)
{
	unsigned int regval, page_off;
	struct nand_chip *chip = host->chip;
	struct async_work_info *current_work;

	if (!chip->async_work_flag) {
		printf("%s error: Not open DMA int flasg\n", __func__);
		return 1;
	}

	/* save information about interrupt at the first time */
	page_off = ((host->addr_value[0] & 0xffff0000)
			>> (16 - chip->page_shift));

	if (chip->complete_flag) {
		current_work = work_info_find_by_id(chip, chip->current_id);
		if (current_work == NULL) {
			printf("%s: Can't find current work!\n", __func__);
			return 2;
		}

		if (current_work->off != page_off) {
			printf("%s error: Not first page, off[%#x] read[%#x]\n",
				__func__, current_work->off, page_off);
			return 0;
		}
	}

	/* clear all interrupt */
	hinfc_write(host, HINFC610_INTCLR_ALL, HINFC610_INTCLR);

	/* enable DMA DONE interrupt */
	regval = hinfc_read(host, HINFC610_INTEN);
	regval |= HINFC610_INTEN_DMA_DONE_EN;
	hinfc_write(host, regval, HINFC610_INTEN);
...
#ifdef CONFIG_SUPPORT_DMA_INT
	host->send_cmd_async_read  = hinfc610_send_cmd_async_read;

	chip->complete_flag = 1;
	chip->async_work_flag = 0;
	chip->current_id = 0;
	INIT_LIST_HEAD(&chip->list);
	chip->work_no = 0;
	chip->total_work_no = 0;
	chip->enable_int = hinfc610_enable_int;
...
int board_nand_init(struct nand_chip *chip)
{
	...
	irq_connect(NFC_IRQ_NUM, hinfc_irq_handle, (void *)&host);
#endif
...
{
	unsigned int regval;
	unsigned int all_page_off;
	unsigned int page_off = 0;
	unsigned int read_len;
	unsigned int page_inside_len;
	unsigned int total_off;
	struct hinfc_host *host = arg;
	struct mtd_info  *nand = host->mtd;
	struct nand_chip *chip = host->chip;
	struct async_work_info *current_work = NULL;
	...
	if (regval & HINFC610_INTS_UE)
		chip->total_status |= HINFC610_PS_UC_ECC;
	...
	all_page_off = ((host->addr_value[0] & 0xffff0000)
			>> (16 - chip->page_shift));
	page_off = current_work->off & (host->pagesize - 1);

	/* First page, alignment offset */
	if (all_page_off != current_work->off - page_off) {
		page_off = 0;
		page_inside_len = host->pagesize;
	} else
		page_inside_len = host->pagesize - page_off;

	total_off = all_page_off - (current_work->off - page_off);

	if (page_inside_len < (host->pagesize - page_off))
		read_len = page_inside_len;
	else
		read_len = host->pagesize - page_off;

	/* save current page date */
	memcpy((void *)(current_work->addr + total_off),
		(void *)(host->dma_buffer + page_off), read_len);
	/* start next page dma read */
	all_page_off += host->pagesize;

	if ((current_work->off + current_work->len)
			<= all_page_off - read_len) {
		struct async_work_info *next = NULL;

		if (current_work->complete)
			current_work->complete(chip->total_status,
					current_work->private);

		next = list_entry(current_work->link.next,
				struct async_work_info, link);

		if (!next) {
			printf("%s error: find next work fail!", __func__);
			return;
		}

		chip->current_id = next->id;

		list_del(&current_work->link);
		free(current_work);
		chip->work_no--;
		...
		if (chip->work_no)
			nand_async_read_list(nand, nand->size + 1, NULL,
					NULL, NULL, NULL);

		return;
	}

	host->addr_value[0] &= 0xffff;
	host->addr_value[0] |= ((all_page_off << (16 - chip->page_shift))
			& 0xffff0000);
